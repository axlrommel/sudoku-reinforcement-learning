...
export const modelCreator = async (req: any, resp: any, next: any) => {
  ...
  const trueObservations: Observation[] = entries.map((e: Entry) => buildObservationFromEntry(e));
  const addedObservations: Observation[] = trueObservations.map((o: Observation) => duplicateBySwitchingRowsAndCols(o));
  const trueRows: any = trueObservations.map((o: Observation) => Object.values(o));
  const addedRows: any = addedObservations.map((o: Observation) => Object.values(o));
